...
public class CheckboxList extends JList<JCheckBox> {
    ...
    public CheckboxList(boolean defaultSelected) {
            ...
            public void mousePressed(MouseEvent e) {
                ...
                if (index != -1) {
                    if (SwingUtilities.isRightMouseButton(e)) {
                        ...
                        popup.add("Select all").addActionListener(e12 -> {
                            for (int i = 0; i < getModel().getSize(); i++)
                                getModel().getElementAt(i).setSelected(true);
                            repaint();
                        });

                        popup.add("Select none").addActionListener(e1 -> {
                            for (int i = 0; i < getModel().getSize(); i++)
    ...
    public static String[] selectOptions(Component parent, boolean defaultSelected, String title, String... options) {
        CheckboxList checkList = CheckboxList.getInstance(defaultSelected, options);
    ...
    public static CheckboxList getInstance(boolean defaultSelected, String... options) {
        ...
        CheckboxList list = new CheckboxList(defaultSelected);
